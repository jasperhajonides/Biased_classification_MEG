...
classifier_output = np.zeros((nr_tps,nsubs,4))
...
for sb_count in range(0, opts['nsubs']):
    print('computing subject {}'.format(sb_count))
    dec_fname = '/Users/jasperhajonides/Documents/EXP8_UpdProtec/scores/S20_200hz_LongPreCuePeriod_TEMP_single_tp_time-0.4_to_0.9_LDA_windowsize_30_pca0.9_steps1_stim_all_chans_test_on_all_trials'
    if  not(os.path.exists(dec_fname)) or overwrite == True:
        ...
        # include trials
        inx =   (df_read['stimulus_nr']==2) #& ~np.isnan(df_read['stim1'])  # & reject_outliers(abs(df_read['error']), m=2) & reject_outliers(df_read['RT'], m=3) # & ~np.isnan(df_read['stim2'])  & (abs(df_read['error']/math.pi*90)<math.pi/4) & (df_read['RT']<6) &  #
        X = X[inx, :, :]
